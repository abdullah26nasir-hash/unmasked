...
// Turn 1: chip, Not sure, flip up/down, end turn
await asker.evaluate(() => [...document.querySelectorAll('button')].find((b) => b.textContent.trim() === 'Plays in the Premier League?')?.click()); await wait(800);
ok('Question chip sends a question', await has(other, 'Plays in the Premier League?'));
await press(other, 'Not sure'); await wait(500); ok('Not sure answers', await has(asker, 'End turn'));
...
await press(asker, 'End turn'); await wait(600); ok('End turn passes the turn', await other.evaluate(() => !!document.querySelector('[role=tablist]')));

// Turn 2: Ask anything, Yes, End turn
await press(other, 'Ask anything'); await other.type('#q', 'Left footed?'); await press(other, 'Ask'); await wait(700);
ok('Ask anything sends a typed question', await has(asker, 'Left footed?'));
await press(asker, 'Yes'); await press(other, 'End turn'); await wait(600);
...
const secretOfOther = aTurn ? null : 'Messi';
